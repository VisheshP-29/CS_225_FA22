...
size_t heap<T, Compare>::parent(size_t currentIdx) const
{
    ...
    return (currentIdx / 2) - (currentIdx / 2) % 1;
}
...
heap<T, Compare>::heap(const std::vector<T>& elems) 
{
    ...
    _elems.push_back(T());
    for(size_t insert = 0; insert < elems.size(); insert++) {
        _elems.push_back(elems[insert]);
    }
    for(size_t sort = parent(_elems.size() - 1); sort > 0; sort--) {
        heapifyDown(sort);
    }
}
