                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    TreeNode* holdSmallest;
    if(mergeQueue.size() == 0) {
        holdSmallest = singleQueue.front();
        singleQueue.pop();
    } else if(singleQueue.size() == 0) {
        holdSmallest = mergeQueue.front();
        mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while((singleQueue.size() + mergeQueue.size()) != 1) {
        ...
        TreeNode* merged = new TreeNode(temp1->freq.getFrequency() + temp2->freq.getFrequency());
        merged->left = temp1;
        merged->right = temp2;
        mergeQueue.push(merged);
    }

    //root_ = (singleQueue.size() == 1) ? singleQueue.front() : mergeQueue.front();
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    TreeNode* tempNode;
    if(bfile.getNextBit()) {
        tempNode = new TreeNode(Frequency(bfile.getNextByte(), 0));
    } else {
        tempNode = new TreeNode(0);
