...
class KDTree
{
  ...
  private:
    ...
    vector<Point<Dim>> allPoints_;
    KDTreeNode* helperConstructor(int dimension, int left, int right);
    void quickSelectHelper(int dimension, int left, int right, int k);
    int quickSelectPartition(int dimension, int left, int right, int pivotIndex);
    Point<Dim> findNearestHelper(KDTreeNode* subroot, Point<Dim> currBest, const Point<Dim> target, int dimension) const;
    double distanceHelper(const Point<Dim> &first, const Point<Dim> &second) const;
