...
std::vector<Edge> NimLearner::playRandomGame() const {
  ...
  while((currVertex != "p1-0") && (currVertex != "p2-0")) {
    int index = rand() % (g_.getAdjacent(currVertex).size());
    Vertex nextVertex = g_.getAdjacent(currVertex)[index];
    Edge holdEdge = g_.getEdge(currVertex, nextVertex);
    path.push_back(holdEdge);
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
  /* Your code goes here! */
  int loseIndex = (path[path.size() - 1].dest == "p1-0") ? 0 : 1;
